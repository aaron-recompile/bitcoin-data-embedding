...
Taproot（BIP 342 Tapscript）做了一件关键的事：

**移除了单个 witness 栈元素的 520 字节限制和脚本总大小的 10,000 字节限制。**

在 SegWit 时代的 P2WSH 中，虽然数据已经在 witness 里，但仍沿用传统脚本引擎规则，
单个 witness 栈元素仍有 520 字节的限制。
...
1. **防止抢跑（front-running）**：如果直接在一笔交易里暴露铭刻内容，
   ...
   **时序细节**：commit 交易必须先确认，reveal 交易才能安全广播。
   如果两笔交易同时广播，reveal 内容在 mempool 中可见，
   矿工或监控者可以抢先构造自己的 commit 承诺相同内容，
   通过排序或更高手续费抢跑。大多数铭刻工具默认等 commit 确认后才广播 reveal。
...
即使是 archival node（保留完整区块历史），使用 `assumevalid` 参数同步时
会跳过历史区块的脚本执行和签名验证。
这意味着节点下载并存储了 witness 数据，但没有亲自验证其合法性。
铭刻数据的"可访问性"和"已验证性"是两个不同层次的保证：
- **可访问性**：节点存储了数据，可以读取
- **已验证性**：节点亲自执行了脚本验证，确认数据合法性

对于依赖 `assumevalid` 同步的节点，铭刻数据虽然可访问，但验证保证依赖于信任链。
...
这个设计回归了早期 colored coin 思路，用 satoshi 本身作为代币载体。

**好处：**
- 状态即 UTXO，转账即花费，无需维护链下状态数据库
- 不同索引器不会产生余额歧义（状态在链上，解析唯一）
- 与比特币原生模型高度一致

**代价：**
- 代币最小单位被锁定为 1 sat，无法细分
- 总量受限于染色聪数量，无法超过比特币总供应量
- 高币价时小额转账可能不经济（需要支付 sat 作为载体）
